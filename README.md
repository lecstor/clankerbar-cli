...
approves it, promoted to a release only by the operator.
...
pushed by the salvage, the same as for a turn-capped `claude` session. Second,
`phases` runs on opencode on that same claim-tracking basis (a multi-phase
sequence is refused only over a harness that observes no claim, which is codex
today), so the per-phase `max_wall_clock` is reachable in a phased config too -
though such a run still has no per-session turn or token cap (see `max_turns`
above), so set this dial deliberately.
